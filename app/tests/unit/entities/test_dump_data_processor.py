...
def test_valid_extract_user_summary(
...
def test_writes_file_dump_user_data(grouped_user_data):
    ...
    file_name = (
        grouped_user_data.user_data.user_basic_data['name'].replace(
            " ", "").lower() + ".txt"
    )
...
def test_writes_file_with_correct_data(grouped_user_data, valided_filtered_data):
