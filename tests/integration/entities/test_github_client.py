...
def test_expected_return_get_repositories_list(valid_username, valid_input_write_github_user_file):
    ...
    username = user_repositories_list[0]['full_name'].split(
        '/')[0]
    repository_name = user_repositories_list[0]['name']

    assert username == valid_username
    assert repository_name in valid_input_write_github_user_file['repositories_name_list']
